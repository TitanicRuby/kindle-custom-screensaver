# E-Reader Screensaver Converter (Web)

A static, browser-only tool for converting your own photos into screensaver images for Kindle, Kobo, and reMarkable devices — no Python, no SSH, no install. This is a more accessible companion to [`batch_convert.py`](../batch_convert.py) in the repo root: same core idea (resize/crop to your device's exact resolution, convert to grayscale, map onto a 16-level palette), but usable by anyone with a browser.

> **Note:** hobby project, not a maintained/audited tool — see the [repo root README](../README.md) for the full disclaimer. This tool only handles the *image conversion* step; you still need to get the resulting PNGs onto your device yourself (see the root README for the Kindle SSH workflow, or your device's own screensaver-folder instructions).
...
- On GitHub, open the [`web-converter` folder](.), click on `index.html`, then click the **download icon** (⬇, near the top-right of the file view — it may be labeled "Download raw file"). Save it wherever you like (Desktop, Downloads, doesn't matter).
...
- Alternative if the above is confusing: on the repo's main page, click the green **Code** button → **Download ZIP**, extract it anywhere, and find `index.html` inside the extracted `web-converter` folder.
...
> **On a phone or tablet:** opening a locally-downloaded HTML file isn't as reliable on mobile (especially iOS) as it is on a desktop. If a hosted version of this tool is available (see [Deploying](#deploying-to-github-pages) below — check the repo for a live link), just visiting that link in your mobile browser is the more reliable path there.
...
- **Center-crop to fill** — crops the edges to match your device's aspect ratio exactly, no distortion, no padding. Same approach `batch_convert.py` uses.
...
This lives in a subfolder (`web-converter/`) rather than the repo root or a `/docs` folder, so the classic "deploy from branch" Pages setting (which only offers `/` or `/docs`) won't serve it directly. The straightforward way to publish just this folder is a GitHub Actions workflow:

1. In the repo, go to **Settings → Pages** and set **Source** to **GitHub Actions**.
2. Add a workflow (e.g. `.github/workflows/pages.yml`) that uploads `web-converter/` as the Pages artifact:

    ```yaml
    name: Deploy web-converter to Pages
...
3. Push to `main` — the workflow builds and publishes automatically, and the Pages URL shows up under **Settings → Pages**.

No build step is needed beyond that — the workflow just uploads the static files as-is.

---

## Third-party code (vendored, not CDN-loaded)

Both are inlined into `index.html` (see [Single file](#e-reader-screensaver-converter-web) above) so the tool has zero runtime network dependencies. Unmodified original copies are kept under `vendor/` purely for license/attribution reference — they aren't loaded by the page:
...
Same as the rest of this repo — see [`LICENSE`](../LICENSE) (MIT).
